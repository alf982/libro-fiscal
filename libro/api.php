...
try {
    ...
    $pdo->exec("CREATE TABLE IF NOT EXISTS inventario_articulos (
        id INTEGER,
        beneficiary_id INTEGER,
        code TEXT NOT NULL,
    ...
    try {
        $stmt = $pdo->query("PRAGMA table_info(asientos_manuales)");
        ...
        if ($hasPrimaryKey) {
            $pdo->exec("DROP TABLE asientos_manuales");
        }
    } catch (Exception $e) {
        // La tabla no existe o error, no hacer nada
    }

    $pdo->exec("CREATE TABLE IF NOT EXISTS asientos_manuales (
        id INTEGER,
        beneficiary_id INTEGER,
        date TEXT NOT NULL,
    ...
    $pdo->exec("CREATE TABLE IF NOT EXISTS cuentas_contables (
        code TEXT NOT NULL,
    ...
    $pdo->exec("CREATE TABLE IF NOT EXISTS balances_guardados (
        id INTEGER,
        beneficiary_id INTEGER,
        type TEXT NOT NULL,
        date TEXT NOT NULL,
...
if ($action === 'load') {
    try {
        ...
        foreach ($beneficiariosRows as $bRow) {
            ...
            // Cargar artículos para este beneficiario
            $stmtArt = $pdo->prepare("SELECT * FROM inventario_articulos WHERE beneficiary_id = ?");
            $stmtArt->execute([$bId]);
            $articulosRows = $stmtArt->fetchAll();
            $articulos = [];
            foreach ($articulosRows as $art) {
                $articulos[] = [
                    'id' => (int)$art['id'],
                    'code' => $art['code'],
                    'description' => $art['description'],
                    'unit' => $art['unit'] ?? 'Unidad',
                    'initial_stock' => (float)$art['initial_stock'],
                    'initial_cost' => (float)$art['initial_cost']
                ];
            }

            // Cargar movimientos para este beneficiario
            $stmtMov = $pdo->prepare("SELECT * FROM inventario_movimientos WHERE beneficiary_id = ?");
            $stmtMov->execute([$bId]);
            $movimientosRows = $stmtMov->fetchAll();
            $movimientos = [];
            foreach ($movimientosRows as $mov) {
                $movimientos[] = [
                    'id' => (int)$mov['id'],
                    'article_id' => (int)$mov['article_id'],
                    'date' => $mov['date'],
                    'type' => $mov['type'],
            ...
            // Cargar asientos manuales para este beneficiario
            $stmtAs = $pdo->prepare("SELECT * FROM asientos_manuales WHERE beneficiary_id = ?");
            $stmtAs->execute([$bId]);
            $asientosRows = $stmtAs->fetchAll();
            $asientos_manuales = [];
            foreach ($asientosRows as $as) {
                $asientos_manuales[] = [
                    'id' => (int)$as['id'],
                    'date' => $as['date'],
                    'concept' => $as['concept'],
            ...
            // Cargar cuentas contables para este beneficiario
            $stmtCuentas = $pdo->prepare("SELECT * FROM cuentas_contables WHERE beneficiary_id = ?");
            $stmtCuentas->execute([$bId]);
            $cuentasRows = $stmtCuentas->fetchAll();
            $cuentas_contables = [];
            foreach ($cuentasRows as $c) {
                $cuentas_contables[] = [
                    'code' => $c['code'],
                    'name' => $c['name']
                ];
            ...
            // Cargar balances guardados para este beneficiario
            $balances_guardados = [];
            try {
                $stmtBG = $pdo->prepare("SELECT * FROM balances_guardados WHERE beneficiary_id = ?");
                $stmtBG->execute([$bId]);
                $balancesRows = $stmtBG->fetchAll();
                foreach ($balancesRows as $bg) {
                    $balances_guardados[] = [
                        'id' => (int)$bg['id'],
                        'type' => $bg['type'],
                        'date' => $bg['date'],
                        'notes' => $bg['notes'] ?? '',
                        'data' => json_decode($bg['data'], true)
                    ];
                }
            } catch (Exception $e) {
                // Ignorar
...
if ($action === 'save') {
    ...
    try {
        ...
        $pdo->exec("DELETE FROM inventario_articulos");
        $pdo->exec("DELETE FROM inventario_movimientos");
        $pdo->exec("DELETE FROM asientos_manuales");
        $pdo->exec("DELETE FROM cuentas_contables");
        $pdo->exec("DELETE FROM balances_guardados");
        ...
        $stmtArt = $pdo->prepare("INSERT INTO inventario_articulos (id, beneficiary_id, code, description, unit, initial_stock, initial_cost) VALUES (?, ?, ?, ?, ?, ?, ?)");

        $stmtMov = $pdo->prepare("INSERT INTO inventario_movimientos (id, beneficiary_id, article_id, date, type, doc_type, doc_number, qty, unit_cost, total_cost, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");

        $stmtAsientos = $pdo->prepare("INSERT INTO asientos_manuales (id, beneficiary_id, date, concept, account, debe, haber) VALUES (?, ?, ?, ?, ?, ?, ?)");
        
        $stmtCuentas = $pdo->prepare("INSERT INTO cuentas_contables (code, beneficiary_id, name) VALUES (?, ?, ?)");

        $stmtBG = $pdo->prepare("INSERT INTO balances_guardados (id, beneficiary_id, type, date, notes, data) VALUES (?, ?, ?, ?, ?, ?)");
        ...
        foreach ($data['beneficiarios'] as $b) {
            ...
            // Insertar artículos del beneficiario
            if (isset($b['articulos']) && is_array($b['articulos'])) {
                foreach ($b['articulos'] as $art) {
                    $stmtArt->execute([
                        (int)$art['id'],
                        $bId,
                        $art['code'],
                        $art['description'],
                        isset($art['unit']) ? $art['unit'] : 'Unidad',
                        isset($art['initial_stock']) ? (float)$art['initial_stock'] : 0.0,
                        isset($art['initial_cost']) ? (float)$art['initial_cost'] : 0.0
                    ]);
                }
            }

            // Insertar movimientos del beneficiario
            if (isset($b['movimientos']) && is_array($b['movimientos'])) {
                foreach ($b['movimientos'] as $mov) {
                    $stmtMov->execute([
                        (int)$mov['id'],
                        $bId,
                        (int)$mov['article_id'],
                        ...
                        $mov['doc_type'],
                        $mov['doc_number'],
                        isset($mov['qty']) ? (float)$mov['qty'] : 0.0,
                        isset($mov['unit_cost']) ? (float)$mov['unit_cost'] : 0.0,
                        isset($mov['total_cost']) ? (float)$mov['total_cost'] : 0.0,
                        isset($mov['notes']) ? $mov['notes'] : ''
                    ]);
                }
            }

            // Insertar asientos manuales del beneficiario
            if (isset($b['asientos_manuales']) && is_array($b['asientos_manuales'])) {
                foreach ($b['asientos_manuales'] as $as) {
                    $stmtAsientos->execute([
                        ...
                        $as['concept'],
                        $as['account'],
                        isset($as['debe']) ? (float)$as['debe'] : 0.0,
                        isset($as['haber']) ? (float)$as['haber'] : 0.0
                    ]);
                }
            }

            // Insertar cuentas contables del beneficiario
            if (isset($b['cuentas_contables']) && is_array($b['cuentas_contables'])) {
                foreach ($b['cuentas_contables'] as $c) {
                    $stmtCuentas->execute([
                        $c['code'],
                        $bId,
                        $c['name']
            ...
            // Insertar balances guardados del beneficiario
            if (isset($b['balances_guardados']) && is_array($b['balances_guardados'])) {
                foreach ($b['balances_guardados'] as $bg) {
                    $stmtBG->execute([
                        (int)$bg['id'],
                        $bId,
                        $bg['type'],
